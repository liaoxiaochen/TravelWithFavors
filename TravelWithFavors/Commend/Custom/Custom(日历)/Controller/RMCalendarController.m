...
@property (nonatomic, strong) RMCalendarModel *selectModel;
...
//    self.selectModel = self.selectStartCalendarModel;
    

    
    self.calendarMonth = [self getMonthArrayOfSelectDate:self.selectModel Days:self.days showType:self.type isEnable:self.isEnable modelArr:self.modelArr];
 ...
 *  @return 数组
 */
- (NSMutableArray *)getMonthArrayOfSelectDate:(RMCalendarModel *)selectModel Days:(int)days showType:(CalendarShowType)type isEnable:(BOOL)isEnable modelArr:(NSArray *)arr
{
    ...
    if (selectModel) {
        ...
        NSString *dayStr= [NSString stringWithFormat:@"%lu-%lu-%lu", selectModel.year, selectModel.month, selectModel.day];
        NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
        [dateFormatter setDateFormat:@"yyyy-MM-dd"];
        [dateFormatter setTimeZone:[NSTimeZone timeZoneForSecondsFromGMT:8]];//解决8小时时间差问题
        selectdate  = [dateFormatter dateFromString:dayStr];
    }
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (kind == UICollectionElementKindSectionHeader){
        ...
        NSString *monthStr = [NSString stringWithFormat:@"%lu",(unsigned long)model.month];
        monthHeader.masterLabel.attributedText = [NSString changeLabelWithMaxString:[NSString stringWithFormat:@"%@月", monthStr] diffrenIndex:monthStr.length maxFont:32 littleFont:16];
        monthHeader.yearLabel.text = [NSString stringWithFormat:@"%lu年",(unsigned long)model.year];//@"日期";
...
- (void)sureButtonAction:(UIBarButtonItem *)item {
    if (self.calendarBlock) {
        self.calendarBlock(self.selectModel);
    }
}
